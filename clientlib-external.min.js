...
var accent_map = {'á':'a', 'é':'e', 'í':'i','ó':'o','ú':'u','Á':'a', 'É':'e', 'Í':'i','Ó':'o','Ú':'u'};
    ...
    return class {
        ...
        onMyDirectvMenuCollapseClicked(event) {
            ...
            if (this.$myDirectvFirstLevelMenu.hasClass(defaults.classes.open)) {
                this.setOpenedHeight(this.$myDirectvFirstLevelMenu, this.$myDirectvFirstLevelMenuItems.length * 60);
            } else {
        ...
        onViewMoreClicked(event) {
            if (window.matchMedia(`(max-width: ${defaults.breakpoints.mobile}px)`).matches) {
...
class AdditionalOptionHeader {
    constructor() {
        this.selector = "header .header-container .main-navigation > ul .nav-item:last-child"
    ...
    fixAdditionalOption() {
        ...
        headerLastChild.addClass('last-child-fix')

        // Verify if the last option has submenu options
        if ($(`${this.selector} ul`).length === 0) {
            let lastChildLink = $(`${this.selector} ${this.lastOptionLink}`)
            // Hide right arrow in mobile menu
            $(`${this.selector} ${this.arrowMobile}`).hide()
            ...
            headerLastChild.hover(() => {
                $(`${this.selector} ${this.subMenuClass}`).hide()
            })
        }
    }
}
